...
class UnitOfWork {
    ...
    expandUniqueProps(entity) {
        return entity.__meta.uniqueProps.map(prop => {
            if (entity[prop.name]) {
                return prop.reference === enums_1.ReferenceType.SCALAR || prop.mapToPk ? entity[prop.name] : entity[prop.name].__helper.getSerializedPrimaryKey();
            }
            return undefined;
        }).filter(i => i);
    }
    ...
    processReference(parent, prop, reference, visited, idx) {
        ...
        if (Utils_1.Utils.isCollection(reference)) {
            reference.getItems(false)
                ...
                (_a = item.__helper).__schema ?? (_a.__schema = parent.__helper.__schema);
            });
